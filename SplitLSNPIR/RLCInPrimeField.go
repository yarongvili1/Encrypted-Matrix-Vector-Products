...
// Transpose C = (-P // I) to C' = (-P^T | I) for easy row combination
func Generate1DDualSpaceRandomMatrix(L, K, p uint32, seed int64) []uint32 {
...
// Transpose D = (I | P) to D' = (I // P^T) for easy row combination
func Generate1DRLCMatrix(L, K, p uint32, seed int64) []uint32 {
	matrix := GenerateRandomColsOfD(L, K, p, seed)
...
func SampleVectorFromNullSpace(L, K, p uint32, seed int64) []uint32 {
	coeff := pir.RandomPrimeFieldVector(K, p)
